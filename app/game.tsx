...
import { Animated, Image, PanResponder, Text, useWindowDimensions, View } from "react-native";
...
export default function Game() {
  ...
  useEffect(() => {
    function spawn() {
      const { y, scale } = spawnPoints[Math.floor(Math.random() * spawnPoints.length)];
    ...
    function loop(ts: number) {
      ...
      setEntities((prev) => {
        ...
        prev.forEach((ent) => {
          ...
          const hit = entRight >= lassoLeft && entLeft <= lassoRight && entBottom >= lassoTop && entTop <= lassoBottom;
